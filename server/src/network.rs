...
pub async fn start_webtransport_server(state: SharedState) -> anyhow::Result<()> {
    ...
    let hash_path =
        std::env::var("CERT_HASH_PATH").unwrap_or_else(|_| "../app/src/cert_hash.ts".to_string());
    ...
    let mut recv_task = tokio::spawn(async move {
        let handle_payload =
            |payload: &[u8],
             st: &SharedState,
             cid: &str|
             -> core::pin::Pin<Box<dyn std::future::Future<Output = ()> + Send>> {
                ...
                Box::pin(async move {
                    if let Ok(client_move) = serde_json::from_slice::<ClientMove>(&payload_vec) {
                        if let Some((old_x, old_y)) =
                            handle_player_move(&st_clone, &cid_clone, client_move).await
                        {
                        ...
                        log::warn!(
                            "Failed to parse ClientMove from payload: {:?}",
                            std::str::from_utf8(&payload_vec)
                        );
                    }
                })
            };
        ...
        let datagram_fut = async {
            loop {
                match connection_recv.receive_datagram().await {
                    Ok(datagram) => {
                        handle_payload(datagram.payload().as_ref(), &state_clone, &client_id_clone)
                            .await;
                    }
                    Err(_) => break,
                }
